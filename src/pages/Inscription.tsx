...
const Inscription: React.FC = () => {
  ...
  React.useEffect(()=>{
    load()
    const ch = supabase.channel('players-all')
  ...
  async function addPlayer(){
    ...
    setSubmitting(false)
    if (error) { console.error(error); return alert("Erreur d'inscription") }
    setPseudo(''); setDpmUrl(''); setDiscord(''); setRank('Unranked')
  }
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {players && !players.length && (
                <tr><td colSpan={5} className="py-6 text-center text-gray-500">Aucune inscription pour l'instant.</td></tr>
              )}
            </tbody>
          </table>
        </div>
